...
class Converter(object):
    ...
    def convert(self, input_data, output_data, format, is_dir=True, **kwargs):
        ...
        if format == Format.JSON:
            ...
            self.convert_to_csv(input_data, output_data,
                                sep=sep, header=header, is_dir=is_dir)
            ...
            self.convert_to_coco(input_data, output_data,
                                 output_image_dir=image_dir, is_dir=is_dir)
            ...
            image_dir = kwargs.get('image_dir')
            self.convert_to_voc(input_data, output_data,
                                output_image_dir=image_dir, is_dir=is_dir)
    ...
    def annotation_result_from_task(self, task):
        ...
        def cancelled(x): return not (x.get('skipped', False)
                                      or x.get('was_cancelled', False))
        annotations = list(filter(cancelled, annotations))
        if not annotations:
            return None

        # sort by creation time
        annotations = sorted(annotations, key=lambda x: x.get(
...
# TODO: Fix BBox dimensions instead of outputting tags only
    def convert_to_csv(self, input_data, output_dir, is_dir=True, **kwargs):
        ...
        pd.DataFrame.from_records(records).to_csv(
            output_file, index=False, **kwargs)
...
# TODO: Fix download
            ...
            label_path = os.path.join(output_label_dir, os.path.splitext(
...
# - WORKAROUND for now by commenting out this, thus no "email" will be generated
            ...
            for bbox in bboxes:
                key = 'rectanglelabels' if 'rectanglelabels' in bbox else (
                    'labels' if 'labels' in bbox else None)
                if key is None:
                    continue
