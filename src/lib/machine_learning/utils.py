...
def get_transform():
    ...
    if session_state.project.deployment_type == 'Object Detection with Bounding Boxes':
        min_area = session_state.new_training.augmentation_dict['min_area']
        min_visibility = session_state.new_training.augmentation_dict['min_visibility']
        transform = A.Compose(
            transform_list,
...
def generate_tfod_xml_csv(image_paths: Path, xml_paths: Path, output_img_dir: Path, csv_path: Path):
    ...
    xml_df = xml_to_df(str(xml_paths))

    logger.info('Generating DataFrame for augmented bounding boxes ...')
